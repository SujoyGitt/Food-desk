...
const Header = ({ sidebartoggle, setsidebartoggle }) => {
  ...
  const [selectLocation, setselectLocation] = useState("India");
  ...
  return (
    <div className={!sidebartoggle?"header w-full h-20  bg-themeorange flex justify-between items-center px-5 fixed top-0 left-0":"header headerAdd w-full h-20  bg-themeorange flex justify-between items-center px-5 fixed top-0 left-0"}>
      <div className="nav-left flex items-center justify-between">
        <div className="logo flex cursor-pointer items-center" onClick={()=> navigate('/')}>
          <svg
            class="logo-abbr"
            width={!sidebartoggle ? "39" : "0"}
            height="31"
            viewBox="0 0 39 31"
            fill="none"
        ...
        <div
          className="nav-control cursor-pointer ml-10"
          onClick={() => setsidebartoggle(!sidebartoggle)}
        >
          <div
            className={
              !sidebartoggle
                ? "nav-control-line w-6 rounded mt-1.5 bg-white "
                : "rotate-45 translate-x-3 translate-y-1 nav-control-line w-4 rounded mt-1.5 bg-white"
            }
          ></div>
          <div className="nav-control-line w-6 rounded mt-1.5 bg-white"></div>
          <div
            className={
              !sidebartoggle
                ? "nav-control-line w-4 rounded mt-1.5 bg-white hover:w-full"
                : "-rotate-45 translate-x-3 -translate-y-1 nav-control-line w-4 rounded mt-1.5 bg-white"
            }
          ></div>
        </div>
      </div>
      <div className="seachBar flex justify-between items-center">
        <div className="filter flex items-center ml-10  py-3 pr-40 rounded-md bg-themelightorange relative">
          <div className="location-filter px-6 w-56 relative">
            <div
              className="location-filter-header flex items-center w-full justify-between text-white"
              onClick={() => setLocationBox(!locationBox)}
            >
              <div className="location">
                <i class="fa-solid fa-location-dot mr-2 text-sm"></i>
                <span className="text-sm">{selectLocation}</span>
              </div>

              <i class="header-select-icon fa-solid fa-sort-down text-xs"></i>
            </div>
            {locationBox && (
              <ul className="w-full absolute py-2 overflow-hidden top-10 bg-white left-0 rounded-md" ref={profileRef}>
                {cities.map((city, index) => {
                  return (
                    <li
                      className="hover:bg-themeloworange hover:text-themeorange text-xs text-themegray px-5 py-2"
          ...
          <div className="w-0.5 h-6 bg-white mr-4"></div>
          <div className="food-filter ">
            <div className="food-filter-header flex items-center">
              <svg
                width="24"
                height="24"
                className="scale-90 "
              ...
              <input
                className="text-white text-sm ml-3 outline-none border-none w-52"
                placeholder="What do want to eat today?"
                onClick={() => setSearchBox(!searchBox)}
              />
            </div>
          </div>
          {searchBox ? (
            <div className="food-options-show absolute top-full left-0 px-7 py-5  bg-white w-full" ref={profileRef}>
              <h3 className="font-semibold">Recently Searched</h3>
              <ul>
                <li>
                  <NavLink>Bekary</NavLink>
                </li>
                <li>
                  <NavLink>Burger</NavLink>
                </li>
                <li>
                  <NavLink>Beverage</NavLink>
                </li>
                <li>
                  <NavLink>Chicken</NavLink>
                </li>
                <li>
                  <NavLink>Pizza</NavLink>
                </li>
              </ul>
              <h3 className="font-semibold my-2">popular Cuisines</h3>
              <div className="popular-cuisines w-full">
                <div className="popular-cuisines-slider overflow-x-hidden w-full flex justify-between items-center">
                  {FoodApi.foods.slice(4, 10).map((e) => {
                    return (
                      <>
                        <div
                          className="cuisines border flex justify-center items-center text-center border-themelightgray"
                          key={e.id}
                        >
        ...
        <div className="profile relative">
          <div
            className={
              showprofile
                ? "profile-header flex justify-between items-center bg-themelightorange px-3 py-2 rounded-md text-white"
                : "profile-header flex justify-between items-center bg-white px-3 py-2 rounded-md rounded-b-none text-black"
            }
          ...
          >
            <img
              ...
              alt=""
            />
            <span className="ml-2 text-sm">Sujoy</span>
            <i
              class={
                showprofile
                  ? "fas fa-chevron-down -rotate-90 ml-4 text-white text-sm"
                  : "fas fa-chevron-down  ml-4 text-black text-sm"
              }
            ></i>
          </div>
          <ul
            ...
            className={
              showprofile
                ? "profileshow absolute px-3 py-2 -translate-y-10 left-0  w-full"
                : "absolute top-full left-0 bg-white px-3 py-2 w-full rounded-b-lg"
            }
          >
            <li>
